    ...
    <div className="space-y-6 animate-fade-in pb-24" style={{ fontFamily: 'Inter, sans-serif' }}>
          ...
          <div className="flex items-center gap-2 mt-2">
             ...
             <div className="flex items-center gap-2">
               ...
               <input 
                ...
                className={`px-4 py-2 rounded-xl font-black text-xs outline-none border-2 transition-all cursor-pointer ${historyDate === getTodayStr() ? 'bg-indigo-50 border-indigo-100 text-indigo-600' : historyDate > getTodayStr() ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-slate-900 border-slate-800 text-white'}`}
               />
               <button 
             ...
             {historyDate < getTodayStr() && <span className="bg-amber-100 text-amber-700 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest">MODO CONSULTA</span>}
          </div>
        </div>
        <div className="flex flex-col md:flex-row gap-3 mt-4 md:mt-0 relative z-10 shrink-0">
          <button 
            onClick={() => window.print()}
            className="flex items-center gap-2 px-6 py-3 bg-white text-slate-900 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-slate-50 transition-all print:hidden"
          >
            <Printer className="w-4 h-4" /> Imprimir Informe
          </button>
          <div className="flex bg-slate-900 p-1.5 rounded-2xl shadow-lg">
            <button onClick={() => setActiveTab('current')} className={`px-4 md:px-6 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${activeTab === 'current' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}>Vista Muelles</button>
            <button onClick={() => setActiveTab('history')} className={`px-4 md:px-6 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${activeTab === 'history' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}>Listado Diario</button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-5 md:p-6 rounded-[2.5rem] border-2 border-slate-100 shadow-sm">
        <div className="flex justify-between items-center mb-4 px-2">
          <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">📝 Observaciones Turno</label>
          {isToday && (
            ...
            return (
              <div key={muelle} className="flex flex-col gap-3">
                <div className="bg-slate-50/50 p-4 md:p-5 rounded-[2.5rem] border border-slate-200/50 shadow-sm flex-1">
                  <h3 className="text-center font-black text-slate-800 uppercase text-[11px] tracking-[0.3em] mb-4">{muelle}</h3>
          ...
          <div className="bg-white rounded-[2.5rem] border-2 border-slate-100 overflow-hidden shadow-sm overflow-x-auto">
             <table className="w-full text-left text-xs min-w-[650px]">
                ...
                <tbody className="divide-y-2 divide-slate-50">
                  {logs.length === 0 ? (
                    <tr><td colSpan={5} className="px-6 py-16 text-center font-black text-[11px] text-slate-300 uppercase tracking-widest">Sin registros</td></tr>
                  ) : logs.map(log => (
                    <tr key={log.id} className="hover:bg-slate-50/30 transition-colors">
                      <td className="px-6 py-4 font-black text-slate-800">{log.dock_id}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase ${log.side === 'single' ? 'bg-slate-100 text-slate-600' : 'bg-indigo-50 text-indigo-600'}`}>
                      ...
                      <td className="px-6 py-4 text-right">
                        <span className={`px-3 py-1 rounded-xl text-[9px] font-black uppercase ${log.status === 'completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
